...
export default class ExpenseDetails extends React.Component {
    ...
    deleteData = async (id, category) => {
        for (let i in details) {
            if (details[i].id == id) {
                details.splice(i, 1);
            }
        }
        let path = id.toUpperCase();
        console.log('a');
        let url = await firebase.storage().ref(`${path}/${path}.jpg`).delete().then(() => {
            console.log('success')
        }).catch((error) => {
            console.log(error.code)
        })
        rootRef.child('Expense').child(firebase.auth().currentUser.uid).child(category).child(id.toUpperCase()).remove(() => {
    ...
    viewPicture = async (id) => {
        let path = id.toUpperCase();
        let url = await firebase.storage().ref(`${path}/${path}.jpg`).getDownloadURL().then((url) => {
            console.log(url);
            this.props.navigation.navigate('ShowPictureScreen',{
                url : url
            });
        }).catch((error) => {
            console.log(error)
            ...
            <View style={styles.descriptionDetail}>
                <View style={{
                    ...
                    <View style={{
                        ...
                        <TouchableOpacity onPress = {() => this.viewPicture(id)}>
                            <MaterialIcons name = 'image' size = {40} style = {{margin : 3}}/>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => this.props.navigation.navigate('ExpenseUpdateScreen', {
                            category : category,
                            id : id,
                        })}>
                            <MaterialIcons name='edit' size={40} style={{ margin: 3 }} />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => this.deleteData(id, category)}>
                            <MaterialIcons name='delete-forever' size={40} style={{ margin: 3 }} />
                        </TouchableOpacity>
    ...
    insertData() {
        return new Promise((resolve) => {
            ...
            rootRef.child('Expense').child(firebase.auth().currentUser.uid).child(category).on('value', (snapshot) => {
                if (snapshot != null) {
                    details.length = 0;
