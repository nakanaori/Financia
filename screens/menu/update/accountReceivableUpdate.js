...
export default class AccountReceivableUpdate extends React.Component{
    ...
    constructor(props){
        ...
        rootRef.child('AccountReceivable').child(firebase.auth().currentUser.uid).child(nama).child(id.toUpperCase()).on('value',(data) => {
            this.state = {
                uang : data.val().uang,
    ...
    handleSubmit = () => {
        ...
        if(newTask.nama.length || newTask.uang.length ) {
            let trId = this.state.id.toUpperCase();
                rootRef.child('AccountReceivable').child(firebase.auth().currentUser.uid).child(newTask.nama).child(trId).set(newTask);
                this.setState({
                    judul : "a",
                    uang : 0,
                    tanggal : "a",
            ...
            <View style = {styles.container}>
                ...
                <View style = {styles.topBar}>
                    ...
                    <View style = {styles.header}>
                        <Text style = {styles.title}>Update Account Receivable</Text>
