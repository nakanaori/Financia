...
import IncomeComponentTitle from './components/incomeComponentTitle'

const { width, height } = Dimensions.get("screen");
const headerHeight = height * 0.125;
const totalIncomeHeight = height * 0.05;
const totalIncomeWidth = width * 0.5;
...
export default class Income extends React.Component {
    ...
    componentDidMount() {
        ...
        rootRef.child('Income').child(firebase.auth().currentUser.uid).on('value', (snapshot) => {
            if (snapshot != null) {
                details.length = 0;
                totalIncome = 0;
                snapshot.forEach((data) => {
                    data.forEach((detail) => {
                        if (this.dateToMonth(detail.val().tanggal) != moment(today).format('MMMM')) {
                            ...
                            historyFirebase.child('Income').child(month + year).once('value', (snapshot) => {
                                if (snapshot != null) {
                                    temp = snapshot.val().uang;
                                }
                            })
                            historyFirebase.child('Income').child(month + year).set({
                                uang: this.currencyToNumber(detail.val().uang) + temp,
                                bulan: month,
                                tahun: year
                            })
                            let trId = detail.val().id;
                            rootRef.child('Income').child(firebase.auth().currentUser.uid).child(detail.val().category).child(trId.toUpperCase()).remove()
                        } else {
        ...
        return (
            <View style={styles.container}>
                <View style={styles.topBar}>
                    ...
                    <TouchableOpacity style={styles.historyButton} onPress={() => this.props.navigation.navigate('IncomeHistoryScreen')}>
                ...
                <View style={styles.body}>
                    <FlatList
                        numColumns={2}
                        data={details}
                        renderItem={({ item }) => <IncomeComponentTitle item={item} navigation={this.props.navigation} />}
                ...
                <View style={styles.bottom}>
                    <View style={styles.incomeTab}>
                        <Text style={{ paddingHorizontal: 12 }} >Total Income</Text>
                        <View style={styles.totalIncome}>
                            <Text>{this.currencyFormat(totalIncome)}</Text>
                        </View>
                    </View>
                    <TouchableOpacity
...
var styles = StyleSheet.create({
    ...
    totalIncome: {
        borderWidth: 1,
        borderColor: 'black',
        height: totalIncomeHeight,
        width: totalIncomeWidth,
        justifyContent: 'center',
        paddingLeft: 10
    },
    bottom: {
